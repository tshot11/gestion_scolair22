...
export function AnimatedStudentCard({
  ...
  return (
    <div
      className={`rounded-2xl border transition-colors duration-200 overflow-hidden flex flex-col justify-between ${isExpanded ? "bg-[#12305A]/45 backdrop-blur-md border-[#94C5FF]/15 shadow-lg shadow-blue-950/30" : "bg-blue-500/10 hover:bg-[#12305A]/45 backdrop-blur-md backdrop-blur-md border-[#94C5FF]/15 hover:border-[#94C5FF]/15 shadow-sm"}`}
    >
      ...
      /><div className="p-4">
        ...
        <div className="flex items-start justify-between gap-3"><div
          ...
          >
            ...
            {/* Student Avatar */}
            <div className="relative shrink-0"><div className="w-11 h-11 rounded-xl overflow-hidden bg-slate-700 flex items-center justify-center text-white font-bold text-xs border border-[#94C5FF]/15 shadow-inner">
              ...
              {/* Status Badge Pin */}
              <div
                className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-[14px] border-2 border-slate-900 ${presenceStatus === "present" ? "bg-blue-600" : presenceStatus === "absent" ? "bg-rose-500" : presenceStatus === "retard" ? "bg-amber-500" : "bg-slate-500"}`}
                title={`Statut: ${presenceStatus}`}
              /></div>
            {/* Main Identification */}
            <div className="min-w-0 flex-1"><h3 className="text-xs sm:text-sm font-bold text-white font-heading truncate">
          ...
          <div className="flex items-center gap-1.5 shrink-0"><span
            ...
            >
              ...
              type="button"
              onClick={() => setIsExpanded(!isExpanded)}
              className="p-1.5 rounded-lg text-blue-300/70  hover:text-white hover:bg-blue-500/20/60 transition"
        ...
        {!isExpanded && (
          <div className="mt-2.5 pt-2 border-t border-[#94C5FF]/15 flex items-center justify-between text-[11px] text-blue-300/70 "><span className="font-mono text-[10px] text-blue-300/70 ">
            ...
            ><span>Dossier</span> <Eye className="w-3 h-3" /></button></div>
        )}
        {/* UNFOLDED ACCORDION CONTENT */}
        {isExpanded && (
          <div className="mt-3 pt-3 border-t border-[#94C5FF]/15 space-y-2">
            
            {/* Extended Details Grid */}
            <div className="space-y-1.5 text-xs">
              
              {/* Date and Place of birth */}
              <div className="flex items-center justify-between p-2 rounded-xl bg-[#12305A]/45 backdrop-blur-md border border-[#94C5FF]/15"><span className="text-blue-300/70  flex items-center gap-1 text-[11px]"><Calendar className="w-3 h-3 text-blue-400 shrink-0" /><span>Naissance :</span></span><span className="text-blue-100 font-medium text-[11px] text-right truncate max-w-[150px]">
                  
                  {eleve.date_naissance} ({eleve.lieu_naissance || "Kinshasa"}
                  )
                </span></div>
              {/* Tutor / Parent */}
              <div className="flex items-center justify-between p-2 rounded-xl bg-[#12305A]/45 backdrop-blur-md border border-[#94C5FF]/15"><span className="text-blue-300/70  flex items-center gap-1 text-[11px]"><User className="w-3 h-3 text-indigo-400 shrink-0" /><span>Tuteur :</span></span><span className="text-blue-100 font-medium text-[11px] truncate max-w-[150px] text-right">
              ...
              <div className="flex items-center justify-between p-2 rounded-xl bg-[#12305A]/45 backdrop-blur-md border border-[#94C5FF]/15"><span className="text-blue-300/70  flex items-center gap-1 text-[11px]"><Award className="w-3 h-3 text-amber-400 shrink-0" /><span>Moyenne :</span></span><div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-2 pt-1"><button
              ...
              ><Eye className="w-3.5 h-3.5" /><span>Dossier & Cotes</span></button><button
                type="button"
                onClick={() => onGenerateCard(eleve)}
                className="p-2 rounded-xl bg-slate-700/80 hover:bg-blue-500/20 text-blue-100 hover:text-white border border-[#94C5FF]/15 transition"
                title="Générer Carte d'Élève"
              ><CreditCard className="w-4 h-4" /></button></div></div>
        )}
      </div></div>
  );
}
