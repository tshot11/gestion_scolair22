  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto pb-24 sm:pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><h2 className="text-xl sm:text-2xl font-extrabold text-white font-heading">
          ...
          </p></div><div className="w-full sm:w-64"><select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {days.map((day) => {
          ...
          return (
            <div
              key={day}
              className="bg-[#12305A]/45 backdrop-blur-md rounded-2xl border border-[#94C5FF]/15 p-4 space-y-3"
            ><div className="flex items-center justify-between pb-2 border-b border-[#94C5FF]/15"><span className="text-sm font-bold text-white font-heading">
                  {day}
                </span><span className="text-xs text-blue-300/70">
                ...
                <div className="space-y-2.5">
                  ...
                  {dayItems.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="p-3 rounded-2xl bg-[#12305A]/45 backdrop-blur-md border border-[#94C5FF]/15 space-y-1.5"
                      ><div className="flex items-center justify-between text-xs"><span className="font-bold text-blue-400 font-mono">
                            
                            {item.heure_debut} - {item.heure_fin}
                          </span><span className="text-[10px] px-2 py-0.5 rounded-[14px] bg-[#12305A]/45 backdrop-blur-md text-slate-700 border border-[#94C5FF]/15">
                            
                            {room ? room.code : "Salle"}
                          </span></div><div className="text-xs font-bold text-white">
                          
                          {course ? course.nom : "Matière"}
                        </div><div className="flex items-center gap-1.5 text-[11px] text-blue-300/70"><GraduationCap className="w-3 h-3 text-purple-400" /><span>
                            {teacher
