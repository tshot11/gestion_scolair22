...
export function UserManagementView() {
  ...
  const handleAdd = (e) => {
    e.preventDefault();
    if (editId) {
      const u = utilisateurs.find(u => u.id === editId);
      if (u) {
        u.nom = form.nom;
        u.email = form.email;
        u.role = form.role;
        if (form.role === 'TUTEUR') {
          u.eleve_id = form.eleve_id;
        } else {
          delete u.eleve_id;
        }
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 min-h-screen text-slate-200">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-blue-200/70 mt-1">Gérez les accès et les rôles (RBAC).</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#12305A]/90 border border-[#94C5FF]/15 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleAdd} className="p-4 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-blue-300 mb-1">{editId ? 'Nouveau mot de passe (optionnel)' : 'Mot de passe provisoire'}</label>
                <input required={!editId} type="text" value={form.password} onChange={e => setForm({...form, password: e.target.value})} className="w-full bg-[#0B1736]/60 border border-[#94C5FF]/15 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500" />
              </div>
              <div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold shadow-lg shadow-blue-600/30 transition-colors">{editId ? 'Enregistrer' : 'Créer'}</button>
              </div>
            </form>
          </div>
        </div>
