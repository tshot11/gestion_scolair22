    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto pb-24 sm:pb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-[#12305A]/45 A]/45 bg-[#12305A]/45 backdrop-blur-md/40 rounded-2xl border border-[#94C5FF]/15/60 p-4 space-y-3">
          ...
          <div className="relative">
            {" "}
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[#F5F9FF]0" />{" "}
            <input
              type="text"
              placeholder="Filtrer les messages..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full bg-[#12305A]/45 A]/45 bg-[#12305A]/45 backdrop-blur-md/60 backdrop-blur-md border border-[#94C5FF]/15 rounded-xl pl-9 pr-3 py-1.5 text-xs text-white placeholder-blue-200/40 focus:outline-none focus:border-blue-500"
            />{" "}
          </div>{" "}
          <div className="space-y-2 max-h-[550px] overflow-y-auto pr-1">
            ...
            {filteredMessages.map((msg) => (
              <div
                ...
                className={`p-3.5 rounded-2xl border transition cursor-pointer space-y-1.5 ${selectedMsgId === msg.id ? "bg-blue-600/20 border-[#94C5FF]/15" : "bg-[#12305A]/45 backdrop-blur-md/70 border-[#94C5FF]/15 C5FF]/15 hover:bg-[#12305A]/45 backdrop-blur-md"}`}
              >
                {" "}
                <div className="flex items-center justify-between text-[10px] text-[#F5F9FF]0">
                  {" "}
        ...
        <div className="lg:col-span-2 bg-[#12305A]/45 A]/45 bg-[#12305A]/45 backdrop-blur-md/40 rounded-2xl border border-[#94C5FF]/15/60 p-6 space-y-4 flex flex-col justify-between">
          {" "}
          {activeMsg ? (
            <div className="space-y-4">
              {" "}
              <div className="pb-4 border-b border-[#94C5FF]/15/60 space-y-2">
                {" "}
                <div className="flex items-center justify-between">
                  {" "}
                  <span className="px-3 py-1 rounded-[14px] bg-blue-500/20 text-blue-300 text-[11px] font-bold border border-[#94C5FF]/15">
                    {" "}
                    Circulaire Officielle{" "}
                  </span>{" "}
                  <span className="text-xs text-[#font-mono">
                    {activeMsg.date}
                  </span>{" "}
                </div>{" "}
              ...
              <div className="text-xs text-[#F5F9FF] leading-relaxed whitespace-pre-line bg-[#12305A]/45 A]/45 bg-[#12305A]/45 backdrop-blur-md/60 backdrop-blur-md/60 p-5 rounded-2xl border border-[#94C5FF]/15">
                {" "}
                {activeMsg.contenu}{" "}
              </div>{" "}
            </div>
          ) : (
            <div className="text-center py-16 text-[#B8C7DF] text-xs">
              {" "}
              Sélectionnez un message pour le lire{" "}
            </div>
          )}{" "}
          <div className="pt-4 border-t border-[#94C5FF]/15/60 flex items-center justify-between text-xs text-[#F5F9FF]0">
            {" "}
            <span className="flex items-center gap-1.5 text-blue-400">
              {" "}
              <CheckCheck className="w-4 h-4" /> Diffusé sur l'application
              mobile des parents{" "}
            </span>{" "}
          </div>{" "}
        </div>{" "}
      ...
      {isComposeOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          {" "}
          <div className="bg-[#12305A]/45 A]/45 bg-[#12305A]/45 backdrop-blur-md/60 backdrop-blur-md border border-[#94C5FF]/15 rounded-2xl w-full max-w-lg shadow-2xl p-6">
            {" "}
            <div className="flex items-center justify-between pb-4 border-b border-[#94C5FF]/15">
              {" "}
            ...
            <form onSubmit={handleSend} className="space-y-4 py-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Parents 6ème Secondaire">
                    Parents 6ème Secondaire
                  </option>{" "}
                  <option value="Parents 4ème Éducation de Base">
                    Parents 4ème Éducation de Base
                  </option>{" "}
                  <option value="Corps Enseignant">
                    Corps Enseignant
                  </option>{" "}
                </select>{" "}
              </div>{" "}
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-[#94C5FF]/15">
                {" "}
                <button
                  type="button"
                  onClick={() => setIsComposeOpen(false)}
