...
export function CommunicationView() {
  ...
  const [selectedMsgId, setSelectedMsgId] = useState(data.messages[0]?.id || 1);
  ...
  const handleSend = (e) => {
    ...
    setForm({ destinataire: 'Tous les Parents d’Élèves', sujet: '', contenu: '' });
  };

  const filteredMessages = data.messages.filter(m => {
    return `${m.sujet} ${m.expediteur} ${m.destinataire} ${m.contenu}`.toLowerCase().includes(search.toLowerCase());
  });

  const activeMsg = data.messages.find(m => m.id === selectedMsgId) || data.messages[0];
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto pb-24 sm:pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400">
            Circulaires officielles, convocations et communications parents-direction
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-800/40 rounded-3xl border border-slate-700/60 p-4 space-y-3">
              ...
              <div
                ...
                className={`p-3.5 rounded-2xl border transition cursor-pointer space-y-1.5 ${
                  selectedMsgId === msg.id
                    ...
                    : 'bg-slate-800/70 border-slate-700/50 hover:bg-slate-800'
                }`}
              >
        ...
        <div className="lg:col-span-2 bg-slate-800/40 rounded-3xl border border-slate-700/60 p-6 space-y-4 flex flex-col justify-between">
          {activeMsg ? (
            <div className="space-y-4">
              <div className="pb-4 border-b border-slate-700/60 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-400 font-mono">{activeMsg.date}</span>
                </div>
                <h3 className="text-lg font-bold text-white font-heading">{activeMsg.sujet}</h3>
                <div className="flex items-center gap-4 text-xs text-slate-300">
                  <div>De : <strong>{activeMsg.expediteur}</strong></div>
                  <div>À : <strong>{activeMsg.destinataire}</strong></div>
                </div>
              </div>

              <div className="text-xs text-slate-200 leading-relaxed whitespace-pre-line bg-slate-900/60 p-5 rounded-2xl border border-slate-800">
                {activeMsg.contenu}
              </div>
            </div>
          ) : (
          ...
          <div className="pt-4 border-t border-slate-700/60 flex items-center justify-between text-xs text-slate-400">
            <span className="flex items-center gap-1.5 text-emerald-400">
              ...
              Diffusé sur l'application mobile des parents
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-lg shadow-2xl p-6">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <h3 className="text-base font-bold text-white font-heading">Nouvelle Circulaire aux Familles</h3>
            ...
            <form onSubmit={handleSend} className="space-y-4 py-4">
              <div>
                ...
                >
                  <option value="Tous les Parents d’Élèves">Tous les Parents d’Élèves</option>
                  <option value="Parents 6ème Secondaire">Parents 6ème Secondaire</option>
                  <option value="Parents 4ème Éducation de Base">Parents 4ème Éducation de Base</option>
                  <option value="Corps Enseignant">Corps Enseignant</option>
                </select>
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Objet du message *</label>
                <input
                  type="text"
                  required
                  value={form.sujet}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Contenu de la circulaire *</label>
                <textarea
                  required
                  rows={4}
                  value={form.contenu}
